"""Paired H050 audit: motion-only versus a 10% learned-probability tie-break."""
...
UPSTREAM_AUDIT_SLUG = "biohub-exp011-audit-loeo-44b6"
SEED = 314159
EXPECTED_EPOCHS = 5
...
PARENT = matches[0]
audit_candidates = [
    INPUT / "notebooks" / "dmitriigluzdov" / UPSTREAM_AUDIT_SLUG,
    INPUT / UPSTREAM_AUDIT_SLUG,
]
audit_matches = [path for path in audit_candidates if path.exists()]
if len(audit_matches) != 1:
    raise FileNotFoundError(
        {"audit_candidates": [str(path) for path in audit_candidates], "matches": [str(path) for path in audit_matches]}
    )
UPSTREAM_AUDIT = audit_matches[0]
...
def graph_for_policy(coords, edges, policy: str, scale):
    if policy in ("registered_hungarian", "registered_prob_hungarian", "registered_weak_hungarian"):
        coords_array = np.asarray(coords, dtype=float)
        scale = np.asarray(scale, dtype=float)
        probability_weight = {
            "registered_hungarian": 0.0,
            "registered_prob_hungarian": 0.7,
            "registered_weak_hungarian": 0.1,
        }[policy]
        linked = []
        ...
        for timepoint in times[:-1]:
            ...
            if probability_weight:
                ...
                if policy == "registered_prob_hungarian":
                    valid = (residual < gate_um) & (learned_probability > 0.0)
                    minimum_score = 0.55
                else:
                    # H050 keeps the complete physical candidate set.  A
                    # missing learned edge contributes zero rather than
                    # deleting the candidate.  The dummy has the weak arm's
                    # score at the same 7 um geometric gate.
...
upstream_selection_path = UPSTREAM_AUDIT / f"loeo_{HOLDOUT_EMBRYO}_selection.json"
if not upstream_selection_path.is_file():
    raise FileNotFoundError(upstream_selection_path)
upstream_selection = json.loads(upstream_selection_path.read_text())
parent_contract_sha256 = hashlib.sha256(contract_path.read_bytes()).hexdigest()
if (
    upstream_selection.get("status") != "selection_frozen_before_confirmation_and_audit"
    or upstream_selection.get("holdout_embryo") != HOLDOUT_EMBRYO
    or upstream_selection.get("weights_sha256") != weight_sha256
    or upstream_selection.get("parent_contract_sha256") != parent_contract_sha256
    or upstream_selection.get("selected_policy") != "registered_hungarian"
    or upstream_selection.get("checkpoint_validation_movies") != checkpoint_names
    or upstream_selection.get("confirmation_movies") != confirmation_names
    or upstream_selection.get("audit_movies") != audit_names
):
    raise RuntimeError({"reason": "upstream selection is incompatible with frozen H050", "selection": upstream_selection})

selected_threshold = float(upstream_selection["selected_threshold"])
if selected_threshold not in CALIBRATION_THRESHOLDS:
...
def evaluate_pair(names):
    ...
    for name in names:
        ...
        save_candidate_cache(name, coords, edges)
...
def summarize_pair(rows):
    ...
    weak = summary["registered_weak_hungarian"]
    delta = {
        key: float(weak.get(key, 0.0) - base.get(key, 0.0))
...
result = {
    ...
    "upstream_selection_sha256": hashlib.sha256(upstream_selection_path.read_bytes()).hexdigest(),
    "selected_threshold": selected_threshold,
    "selected_policy": upstream_selection["selected_policy"],
    ...
    "promotion_contract": "nonnegative on each untouched fold and positive pooled; no weight or threshold reselection",
    "confirmation_movies": confirmation_names,
    "audit_movies": audit_names,
    "confirmation_summary_by_arm": confirmation_summary,
    "confirmation_per_movie_by_arm": confirmation_rows,
    "confirmation_delta_weak_minus_motion": confirmation_delta,
    "audit_summary_by_arm": audit_summary,
...
result_path = WORK / f"loeo_{HOLDOUT_EMBRYO}_model_comparison_result.json"
result_path.write_text(json.dumps(jsonable(result), indent=2), encoding="utf-8")
print(json.dumps(jsonable({"audit_delta": audit_delta, "fold_nonnegative": fold_nonnegative}), indent=2))
