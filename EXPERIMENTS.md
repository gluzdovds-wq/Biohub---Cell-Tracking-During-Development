...
| EXP-009 | 2026-08-22 | support-pack `train_unet_transformer.py` | Exact TemporalUNet3D + node-transformer training, hold out embryo `44b6`; deterministic 8-movie calibration subset and frozen remainder for audit only. | LOEO contract frozen before training | not a test submission | target 5 epochs, dual T4 | v1 OOM; v2 cancelled as runtime-infeasible; v3 running | Seed `314159`; training contains only `6bba_*`. Live v2 telemetry showed 1,549 steps/epoch at ~3.8 s/step, so 50 epochs could not fit the 12h limit. V3 uses 5 full epochs and 4 checkpoint-validation movies (~7,745 optimizer steps); all 8 calibration movies remain frozen for EXP-011 selection and audit membership is unchanged. |
| EXP-010 | 2026-08-22 | support-pack `train_unet_transformer.py` | Reciprocal exact architecture fold, hold out embryo `6bba`; otherwise identical to EXP-009. | LOEO contract frozen before training | not a test submission | target 10 epochs, dual T4 | v1 OOM; v2 cancelled as runtime-infeasible; v3 running | Seed `314159`; training contains only `44b6_*`. Live v2 telemetry showed 790 steps/epoch at ~3.8 s/step. V3 uses 10 full epochs and the same 4/8 checkpoint/calibration contract (~7,900 optimizer steps), compute-matched to EXP-009. |
