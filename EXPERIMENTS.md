...
| H-015 | Training the exact public `0.923` architecture with embryo-separated folds gives directionally reliable model-selection evidence and exposes public-weight leakage. | Two reciprocal folds: train on one embryo, select checkpoint on 8 frozen movies of the other, preserve all remaining held-out movies for audit-only inference. | Per-fold official metric on never-loaded audit movies, detector recall, edge/division TP-FP-FN and fold variance. | EXP-009/010 running |
...
| EXP-009 | 2026-08-22 | support-pack `train_unet_transformer.py` | Exact TemporalUNet3D + node-transformer training, hold out embryo `44b6`; deterministic 8-movie calibration subset and frozen remainder for audit only. | LOEO contract frozen before training | not a test submission | target 50 epochs, dual T4 | v1 OOM; v2 running | Seed `314159`; training contains only `6bba_*`; audit movies are recorded in the output contract and never enter the trainer split. V1 failed on the first batch because batch `16` required another 4.02 GiB with only 2.93 GiB free. V2 changes only batch `16 -> 8`, workers `8 -> 4`, and the allocator fragmentation guard; it passed the prior failure-time boundary. |
| EXP-010 | 2026-08-22 | support-pack `train_unet_transformer.py` | Reciprocal exact architecture fold, hold out embryo `6bba`; otherwise identical to EXP-009. | LOEO contract frozen before training | not a test submission | target 50 epochs, dual T4 | v1 OOM; v2 running | Seed `314159`; training contains only `44b6_*`; same checkpoint-selection and audit policy as EXP-009. Applies the identical memory-only v2 correction and passed the prior failure-time boundary. |
| EXP-011 | 2026-08-22 | EXP-009 checkpoint | Holdout-`44b6` official-metric audit: choose detection threshold from `0.95/0.97/0.985/0.99/0.995` and linker from greedy/public-ILP/support-ILP/registered-Hungarian on the frozen calibration eight, persist the pair, then evaluate the untouched audit remainder once. | strict held-out audit contract | not a test submission | target T4 | prepared | UNet candidates are shared across learned linker policies; the physical arm uses the same detections. No repair or audit-driven sweep. Depends on successful EXP-009 completion. |
| EXP-012 | 2026-08-22 | EXP-010 checkpoint | Reciprocal holdout-`6bba` official-metric audit with exactly the EXP-011 protocol. | strict held-out audit contract | not a test submission | target T4 | prepared | Provides fold variance and a non-leaking gate for later TTA/repair experiments. Depends on successful EXP-010 completion. |
| EXP-013 | 2026-08-22 | support-pack runtime | CPU-only environment smoke test for the exact `build_graph`, public/support ILP policies, degree constraints and GEFF save/reload path used by EXP-011/012. | synthetic graph; no model/data score | not a test submission | <1 min CPU | complete v1 | PASS: both solver paths and GEFF round-trips succeeded; support-ILP selected 6 nodes/3 edges with degree `1/1`. Public-ILP legally selected an empty graph on the two-frame toy because its disappearance cost exceeds the short-path edge reward; real calibration will reject it if degenerate. Receipt SHA-256 `70348d12…a07`. |
