...
| EXP-019 | prepared 2026-08-22 | exact EXP-006 artifact + raw test volumes | CPU-only gated intensity-COM coordinate refinement; no node, time or edge changes. | public weights; mechanism independently used by EXP-008 | reserve behind EXP-014; no automatic submission | projected CPU-only | prepared | Source must match SHA-256 `5c852379…1f4d`. Fixed raw-shift gate `1.5 µm` and `alpha=0.35` cap output movement at `0.525 µm`; emits a provenance/displacement receipt and fails on topology drift. |
