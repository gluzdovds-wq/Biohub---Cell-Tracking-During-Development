...
| H-019 | Unanimous alternative associations from two independent trackers can correct wrong partners in the strongest graph at very high precision. | Map EXP-007 and EXP-008 nodes mutually to EXP-006 within `2 µm`; replace a one-to-one EXP-006 edge only when both complete mapped outgoing sets contain the same single different target. | Number of unanimous alternatives, degree/schema invariants and LB delta; never alter divisions, nodes or coordinates. | EXP-015 PASS; only one accepted edge, hold for combination |
...
| EXP-014 | 2026-08-22 | EXP-006 topology + EXP-008 coordinates | Coordinate-only ensemble: mutual-nearest same-frame detections within `2 µm`, fixed `alpha=0.5` convex blend; no node, time or edge changes. | public weights; topology inherited from clean `0.923` source | next available slot candidate | 11 s local | artifact complete v1 | 93,630/122,266 nodes matched (`76.58%`); mean detector disagreement `0.999 µm`, p95 `1.773 µm`, so the base graph moves only `0.499/0.887 µm` at mean/p95. Audit PASS: 122,266 nodes, 118,156 edges, max degrees 1/2. SHA-256 `6fcc8d22…2109`. |
| EXP-015 | 2026-08-22 | EXP-006 + mapped EXP-007/008 edges | Edge-only unanimous correction: replace a base one-to-one edge only when both independent trackers have complete mapped outgoing sets and agree on the same different target. | public weights; high-precision consensus diagnostic | hold; not worth a standalone slot | 13 s local | artifact complete v1 | 111,374 EXP-007 and 93,630 EXP-008 nodes map mutually to EXP-006. Only 2 unanimous alternatives exist; 1 is rejected for target-parent conflict and 1 edge changes (`44b6_0113de3b: 21133→21434` to `21133→21435`). Audit PASS, node/edge counts unchanged, SHA-256 `4c1ee557…4278`. |
...
| EXP-018 | 2026-08-22 | EXP-015 topology + EXP-008 coordinates | Exact composition of the fixed `alpha=0.5`, `2 µm` coordinate ensemble and the single unanimous edge correction. | public weights; topology inherited from EXP-015 | reserve; submit only after EXP-014 establishes coordinate effect | 11 s build + 26 s audit | artifact complete v1 | Audit PASS: 122,266 nodes, 118,156 edges, max degrees 1/2; 93,630 coordinates refined and only the pre-audited `21133→21435` edge differs from EXP-014. SHA-256 `b772ef3e…9853`. |
| EXP-019 | 2026-08-22 | exact EXP-006 artifact + raw test volumes | CPU-only gated intensity-COM coordinate refinement; no node, time or edge changes. | public weights; mechanism independently used by EXP-008 | reserve behind EXP-014; no automatic submission | CPU-only | running v1 | Kaggle `dmitriigluzdov/biohub-exp019-intensity-coordinate-refine`. Source must match SHA-256 `5c852379…1f4d`; fixed raw-shift gate `1.5 µm` and `alpha=0.35` cap output movement at `0.525 µm`; emits a provenance/displacement receipt and fails on topology drift. |
