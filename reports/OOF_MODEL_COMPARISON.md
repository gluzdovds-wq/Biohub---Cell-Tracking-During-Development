...
EXP077 v1 is COMPLETE, checked 2026-08-28: four full hash-selected audit movies on Kaggle CPU, no flip TTA, shared candidates for registered/weak/local-deformation/ILP controls. Model and split contracts passed; GPU was disabled and local/remote source matched. Total time **20.30 minutes**, zero failed movies. Our leave-self-out local deformation linker has paired delta **0.0 on both embryos and pooled** against the weak control. Pooled registered/weak/local-flow/ILP scores are `0.624991 / 0.624970 / 0.624970 / 0.613500`. No GT divisions occur; division robustness is untested. This establishes feasibility, not improvement or complete OOF. Four compressed candidate caches (1.08 MB total) are now local and checksum-verified, with no image download. See `CPU_LOCAL_FLOW_PILOT.md` and `cpu_pilot_result_20260828.json`.

Measured per-movie times imply roughly 1.94 CPU wall hours for 24 movies or 14.83 hours for 183, excluding repeated setup; these are four-movie extrapolations, not exact-model benchmarks. A bounded CPU continuation is feasible and does not need to wait for GPU quota. It has not yet been launched.
